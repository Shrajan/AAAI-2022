...
def DoG_OOCS_3D(x, y, z, center, gamma, radius):
...
def On_Off_Center_filters_3D(radius, gamma, in_channels, out_channels, off=false):
    ...
    for i in range(kernel_size):
        for j in range(kernel_size):
            for k in range(kernel_size):
                excite, inhibit = DoG_OOCS_3D(i + 1, j + 1, k + 1, centerX, gamma, radius)
                if excite > inhibit:
                    posExcite += excite
                    posInhibit += inhibit
                else:
